...
FileInstall,gmhat.png,%A_ScriptDir%\Assets\gmhat.png,1
...
gui, add, checkbox, x65 y180 galarms checked, Alarms
Gui, add, text, x57 y210 c0000A0, GMSReboot
global hid := 2
gosub,alarms
    ...
    atk() {
        ...
        loop{
            ...
            while Errorlevel = 0 {
                ...
                sleep 100
                ImageSearch,vX,vY,1,1,400,400, *1 me.png
                if(vX < 143 && Errorlevel = 0) {
                    while (vX < 143 && Errorlevel = 0) {
                ...
                loop 12 {
                    random, ran, 88,115
                    AHI.Instance.SendKeyEvent(HID, GetKeySC("w"), 1)
                    sleep %ran%
                    AHI.Instance.SendKeyEvent(HID, GetKeySC("w"), 0)
                    sleep 100
                }
                sleep 1000
                if(loot) {
...
Transcendent:
    Transcendent()
return
Transcendent() { ; Transcendent, check for Boss drop
    AHI.Instance.SendKeyEvent(HID, GetKeySC("Left"), 0)
    AHI.Instance.SendKeyEvent(HID, GetKeySC("Right"), 0)
    AHI.Instance.SendKeyEvent(HID, GetKeySC("Up"), 0)
    AHI.Instance.SendKeyEvent(HID, GetKeySC("Down"), 0)
...
return
gm() {
    ImageSearch,RuneX,RuneY,1,1,1920,1080, *25 gmhat.png
    if(Errorlevel = 0) {
        AHI.Instance.SendKeyEvent(HID, GetKeySC("Left"), 0)
        AHI.Instance.SendKeyEvent(HID, GetKeySC("Right"), 0)
        AHI.Instance.SendKeyEvent(HID, GetKeySC("Up"), 0)
