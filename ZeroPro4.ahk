...
f3::
    atk(){
        ...
        SetTimer, ArrowRainInhuman, -130000
        ...
        loop{
            ...
            if (clockwise) {
                ...
                while(vX < RightX && Errorlevel = 0) { ;Character going right till X wall is met
                    AHI.Instance.SendKeyEvent(HID, GetKeySC("Right"), 1)
                    AHI.Instance.SendKeyEvent(HID, GetKeySC("space"), 1)
                    sleep 50
                    ImageSearch,vX,vY,1,1,400,400, *1 me.png
                    if(vX > 125 && Errorlevel = 0){
                        if(ShadowFlash) { ;skill when off cooldown
                            AHI.Instance.SendKeyEvent(HID, GetKeySC("Right"), 0)
                            ImageSearch,vX,vY,1,1,400,400, *1 me.png
                            sleep 200 
                            loop 4{
                ...
                if(vertical) {
                    if(vY > TopY && Errorlevel = 0) { ;at the end of Rightcycle, rope lift Up after moving left if beyond boundry
                        ImageSearch,vX,vY,1,1,400,400, *1 me.png
                        vX2 := vX - 3
                        while(vX > vX2 && Errorlevel = 0) {
                            AHI.Instance.SendKeyEvent(HID, GetKeySC("Left"), 1)
                ...
                ImageSearch,vX,vY,1,1,400,400, *1 me.png
                while(vX > LeftX && Errorlevel = 0) { ;Character going left till X wall is met
                    Gosub, ZeroBetaLeft
                    ImageSearch,vX,vY,1,1,400,400, *1 me.png
                    sleep 50
                    if(beta) {
                    ...
                    ImageSearch,vX,vY,1,1,400,400, *1 me.png    
                    if (vY <= 124 && vX < 148 && ErrorLevel = 0){ ;if Character is on platform 124,148
                ...
                if(vertical) {
                    while(vY < TopY && Errorlevel = 0) { ;Character top of map, to jump down
                        critical, on
                        AHI.Instance.SendKeyEvent(HID, GetKeySC("Down"), 1)
                        sleep 50
                        AHI.Instance.SendKeyEvent(HID, GetKeySC("right"), 1)
...
f12::
    Winactivate, ahk_class MapleStoryClass
    ImageSearch,vX,vY,1,1,400,400, *1 me.png
    MsgBox, character is at X%vX% Y%vY%.
Return